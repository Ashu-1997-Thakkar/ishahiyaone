...
$sql = "
    SELECT SQL_CALC_FOUND_ROWS * FROM (
        ...
        UNION ALL
        SELECT id AS product_id, CAST(name AS CHAR CHARACTER SET utf8mb4) AS name, CAST(brand AS CHAR CHARACTER SET utf8mb4) AS brand, price, CAST(Image1 AS CHAR CHARACTER SET utf8mb4) AS Image1, id AS cat_id, 'subcategories' AS source, created_at, bumper_end_date, bumper_discount FROM subcategories WHERE is_bumper_offer = 1 AND (bumper_start_date IS NULL OR bumper_end_date IS NULL OR CURDATE() BETWEEN DATE(bumper_start_date) AND DATE(bumper_end_date))
        UNION ALL
        SELECT product_id AS id, CAST(name AS CHAR CHARACTER SET utf8mb4) AS name, CAST(brand AS CHAR CHARACTER SET utf8mb4) AS brand, price, CAST(image AS CHAR CHARACTER SET utf8mb4) AS Image1, COALESCE(sub_category_id, category_id) AS cat_id, 'products' AS source, created_at, bumper_end_date, bumper_discount FROM products WHERE is_bumper_offer = 1 AND (bumper_start_date IS NULL OR bumper_end_date IS NULL OR CURDATE() BETWEEN DATE(bumper_start_date) AND DATE(bumper_end_date))
    ) as combined_offers
